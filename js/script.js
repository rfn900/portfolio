document.addEventListener("DOMContentLoaded", () => {
  ...
  const scroll_button_link = document.getElementById("scroll-button-link");
    ...
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        ...
        if (entry.target.id == "about-section") {
          document.querySelector(".img-wrapper").style.transform =
            "translateX(0%)";
          document.querySelector(".main-content .text").style.transform =
            "translateX(0%)";
          document.querySelector(".card-icons").style.transform =
            "translateZ(60px) translate(10px,130px) rotateZ(-2deg)";
        }
  ...
  const sectionOneObserver = new IntersectionObserver(function (
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        document.querySelector(".menulist span").style.backgroundColor =
          "#ffffff";
        document
          .querySelector(".scroll-up-button")
          .classList.remove("scroll-up-pop");
      } else {
        if (!nav_toggle_checkbox.checked) {
          document
            .querySelector(".scroll-up-button")
            .classList.add("scroll-up-pop");
  ...
  nav_toggle_checkbox.addEventListener("change", () => {
    ...
    document
      ...
      .classList.toggle("navbar-menu-collapse");
  });
  ...
  document.querySelectorAll(".navbar-menu li a").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      document
        .querySelector(".navbar-menu")
        .classList.remove("navbar-menu-collapse");
    });
  });
  ...
  close_span.addEventListener("click", () => {
    document
      .querySelector(".program-description-modal")
  ...
  scroll_button_link.addEventListener("click", () => {
